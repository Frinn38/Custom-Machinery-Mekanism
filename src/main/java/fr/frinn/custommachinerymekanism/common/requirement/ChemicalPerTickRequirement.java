...
public record ChemicalPerTickRequirement(RequirementIOMode mode, Chemical chemical, long amount, String tank) implements IRequirement<ChemicalComponentHandler>, IJEIIngredientRequirement<ChemicalStack> {

    public static NamedCodec<ChemicalPerTickRequirement> CODEC = NamedCodec.record(chemicalRequirementInstance ->
            ...
            ).apply(chemicalRequirementInstance, ChemicalPerTickRequirement::new), "Chemical requirement"
    );
    ...
    public List<IJEIIngredientWrapper<ChemicalStack>> getJEIIngredientWrappers(IMachineRecipe recipe, RecipeRequirement<?, ?> requirement) {
        return Collections.singletonList(new ChemicalIngredientWrapper(this.mode, this.chemical, this.amount, requirement.chance(), true, this.tank));
    }
}

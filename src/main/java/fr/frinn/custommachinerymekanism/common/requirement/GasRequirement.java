...
public class GasRequirement extends ChemicalRequirement<Gas, GasStack, GasComponentHandler> {
    ...
    @Override
    public List<IJEIIngredientWrapper<GasStack>> getJEIIngredientWrappers(IMachineRecipe recipe, RecipeRequirement<?, ?> requirement) {
        return Collections.singletonList(new GasIngredientWrapper(this.getMode(), this.chemical, this.amount, requirement.chance(), false, this.tank));
    }
}
